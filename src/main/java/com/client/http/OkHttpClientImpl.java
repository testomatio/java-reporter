...
import com.exception.ApiRequestFailedExeption;
...
import static com.constants.CommonConstants.HTTP_TIMEOUT_SECONDS;
...
//            if (!response.isSuccessful()) {
//                LOGGER.error("API request failed: HTTP {} - {} | URL: {} | Response: {}",
//                        response.code(), response.message(), requestUrl, responseBodyString);
//                throw new ApiRequestFailedExeption("API request failed: " + "-> " + request.url() + "<-" + response.code() + " " + response.message());
//            }
//
//            if (responseType == null) {
//                return null;
